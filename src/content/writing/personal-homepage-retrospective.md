---
title: "个人主页：从展示页到长期整理空间"
description: "一次关于个人主页定位、内容结构和技术取舍的整理。"
date: 2026-05-18
tags: ["Personal Site", "Writing", "Projects"]
status: published
category: "项目复盘"
topics:
  - personal-systems
  - writing
---

## 为什么做这个主页

最开始，我只是想给自己做一个更清晰的线上入口。

过去一段时间里，我陆续整理过一些项目、笔记和方法，但它们分散在不同地方：有些在本地文件里，有些在 GitHub 仓库里，有些只是阶段性的想法。时间久了，这些内容很容易变成一次性的记录，很难被重新看见，也很难形成连续的线索。

所以，我希望有一个长期更新的个人主页，用来放置这些内容：项目、写作、方法、工具实践，以及一些和学习、工作流有关的阶段性思考。

它不需要承载太多即时目标。我更需要的是一个稳定的整理空间，能把这些分散内容慢慢接起来。

## 从展示页转向整理空间

做个人主页时，很容易把它做成一个"展示自己"的页面：放经历、放标签、放项目链接，然后尽量让页面看起来完整、漂亮、专业。

但做着做着，我发现自己并不想把它只做成展示页。

我更希望它成为一个能够持续生长的空间。项目可以被复盘，笔记可以被整理，方法可以被迭代，阶段性的判断也可以被保留下来。相比一次性把自己包装完整，我更在意这个页面能否长期记录真实做过的事情，以及这些事情背后的思考方式。

所以后面调整方向时，我更关注它能不能记录我正在关注什么、做过什么、如何整理问题，以及这些内容如何随着时间慢慢变化。

<figure>
  <button type="button" class="article-image-preview" data-preview-src="/images/writing/personal-homepage/homepage-overview.webp" data-preview-alt="个人主页首页整体视图" aria-label="预览首页整体视图大图">
    <img src="/images/writing/personal-homepage/homepage-overview.webp" alt="个人主页首页整体视图" loading="lazy" />
  </button>
  <figcaption>图 1：首页整体视图，用于说明当前个人主页的主要结构。</figcaption>
</figure>

## 这次整理解决了什么问题

这次整理先处理的是入口。以前内容分散时，访问者很难快速判断这个主页到底在关注什么，只能看到一组零散链接。现在首页保留 About、Topics、Notes、Builds、Recent 和 Contact 几个区域，各自承担不同功能：About 说明我是谁和在做什么，Topics 提供主题入口，Notes 放方法和观察，Builds 放项目复盘，Recent 留下近期节点，Contact 保持简单的联系入口。

接着是把不同类型的内容拆开。项目、文章和时间线如果混在一起，后续会很难维护：项目需要完整复盘，文章适合承载方法和思考，时间线则记录阶段性变化。拆开之后，我在补内容时会更清楚一件事应该放在哪里，也更容易判断它是不是值得长期保留。

表达方式也顺手重新收了一遍。我不想把页面写成很用力的个人包装，所以整体语气尽量保持克制，只把做过的事情、形成过的方法和当时的取舍讲清楚。

## 内容结构如何设计

目前这个主页主要由几类内容组成。

**Notes** 用来整理方法、观察和阶段性思考。相比短内容，我更希望这里的文章有一定的上下文，能够在未来重新阅读时仍然有意义。

**Topics** 用来呈现我长期关注的方向，包括 AI Tools、Digital Products、Personal Systems、Writing 和 Learning。这些不是固定标签，更像是内容的入口。后来我把 Topics 从单纯的静态入口页，改成了 Related Projects / Related Notes 的内容索引机制，这样每个主题下面能看到真正相关的项目和文章，而不是只停留在一个标题列表。

**Builds** 用来放置项目复盘。这里不会只放结果，也会记录项目为什么开始、解决了什么问题、做了哪些取舍。最近 Works / Projects / Recent 也补齐了三个 AI Demo 的内容，让主页不只是结构上完整，而是真的有项目经验可以被串起来。

**Recent** 用来记录一些关键节点。它不追求完整记录所有事情，而是保留那些对方向变化、项目推进或方法形成有意义的片段。

<figure>
  <button type="button" class="article-image-preview" data-preview-src="/images/writing/personal-homepage/content-structure.webp" data-preview-alt="首页信息架构示意图" aria-label="预览首页信息架构示意大图">
    <img src="/images/writing/personal-homepage/content-structure.webp" alt="首页信息架构示意图" loading="lazy" />
  </button>
  <figcaption>图 2：首页内容结构示意，Notes 为核心内容入口，Topics 用于组织主题。</figcaption>
</figure>

这些结构并不复杂，但足够支撑一个长期更新的个人主页。

## 技术上的克制

这个主页使用 Astro、TypeScript、Tailwind CSS，以及 Markdown / MDX 内容结构。

选择这套技术栈的原因很简单：它足够轻量，也足够适合内容型个人主页。页面可以保持静态生成，内容可以通过 Markdown 持续补充，样式也不需要依赖复杂组件库。

这次我刻意避免加入数据库、登录、后台系统、复杂表单或 AI API。对于当前阶段来说，这些功能不会明显提高主页价值，反而会增加维护成本。

长期维护下来，结构清晰、更新方便、后续不容易失控，比功能复杂更重要。

<figure>
  <button type="button" class="article-image-preview" data-preview-src="/images/writing/personal-homepage/mobile-reading-view.webp" data-preview-alt="移动端阅读场景示意" aria-label="预览移动端阅读场景示意大图">
    <img src="/images/writing/personal-homepage/mobile-reading-view.webp" alt="移动端阅读场景示意" loading="lazy" />
  </button>
  <figcaption>图 3：移动端阅读场景示意，用于观察内容区块和正文节奏。</figcaption>
</figure>

## 后续会如何更新

接下来，这个主页会以比较慢的节奏继续更新。

我会逐步补充更多项目复盘，整理一些关于 AI 工具、数字产品、个人工作流和学习方法的笔记，也会保留一些阶段性的判断和变化。

这个页面不会追求频繁更新，也不需要一次性变得完整。它更像是一个长期容器：随着项目推进、方法积累和认知变化，慢慢沉淀出更清晰的内容。后面每次重要修改，我还是会先本地 build 通过，再 commit，尽量让这个空间保持可回滚、可继续整理的状态。
